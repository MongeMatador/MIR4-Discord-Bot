    ...
    async def dispatch_claim_log(self, guild_id: int, user: discord.User, spot_name: str, action: str):
        ...
        if not channel:
            try:
                ...
                logger.error(f"[LOG SERVICE] Não foi possível encontrar o canal ID {channel_id}: {e}")
                return
        ...
        try:
            ...
            logger.info(f"[LOG SERVICE] Log enviado para o canal {channel_id}.")
        except Exception as e:
            logger.error(f"[LOG SERVICE] Falha ao enviar log: {e}")
